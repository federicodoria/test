# ===========================================================================
# MASONRY FRAME — GRAVITY + SPRING SETTLEMENT (END PIER) + HORIZONTAL PUSHOVER
# Three-Storey Building, 3 Piers / 2 Bays
# ===========================================================================
#
# Column layout (plan, along X):
#   Left pier    x = 0            <-- settlement applied here
...
#   28     : dummy fixed node under the left base, for the settlement spring
#
# Element numbering:
#    1- 3  : left column piers,   storeys 1-3   (bear on the settling spring)
...
#   20     : zeroLength settlement spring under the left base (removed after gravity)
# ===========================================================================
...
# Reaction estimate at the LEFT (end) base node: its own column (3 piers) +
...
puts "Estimated reaction at left base node (node 1) : [format %.0f $R_est] N"
...
for {set sit 0} {$sit < $max_sit} {incr sit} {
    ...
    element zeroLength 20  1 28  -mat 1  -dir 3

    # --- left column (piers 1-3) — settling column ---
...
loadConst -time 0.0
set d_final [nodeDisp 1 3]
puts "\nFinal settlement at left base node (node 1): [format %.5f $d_final] m"
...
recorder Node -file SettlementDisp.out   -time -node 1            -dof 3   disp
